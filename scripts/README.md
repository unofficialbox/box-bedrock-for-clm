...
| `tools/box-capture/forms.py` | Build an explicitly acknowledged, non-destructive browser executor for the isolated CLM Forms private-API lab; never targets the production Form |
| `tools/box-capture/apps.py` | Build an explicitly acknowledged browser executor for the isolated Apps private-API lab; now includes section-aware reconciliation in addition to shell/description; never deletes, publishes, shares, or targets the production App |
| `tools/box-capture/automate.py` | Build an explicitly acknowledged executor for either the empty inactive Automate lab or the separate Manual Start-only graph lab; never deletes, publishes, activates, shares, or runs either lab. `--write-inspector` builds a read-only reader for an existing unpublished draft that issues no GraphQL operation and mutates nothing |
