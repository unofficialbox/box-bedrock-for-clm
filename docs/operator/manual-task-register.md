...
| MT-047 | Create a Lightning Out 2.0 app for the site and record its 18-digit id | Salesforce admin | Required for live Agentforce | **Setup → Lightning Out 2.0 App Manager**. The org has no `LightningOutApp` record, so Lightning Out has no app to load and the conversation panel mounts empty. Set `site-prefix` to `/clm`, then supply the id as `VITE_AGENTFORCE_APP_ID` or `agentforceAppId` in runtime config. `LightningOutApp` is not a deployable metadata type, so this cannot be scripted |
