...
| MT-047 | Create the Agentforce app deployment and record its app id | Salesforce admin | Required for live Agentforce | The conversation client needs both an agent id and an app id. This org has no EmbeddedServiceConfig or MessagingChannel, so the deployment must be created before the panel can render a conversation |
