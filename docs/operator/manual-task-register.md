...
| MT-039 | Set the seven `CLM_Box_Config__c` values with `configure-clm-box-settings.sh` | Salesforce admin | Required for live Box preview | Values come from `config/deploy/environment.local.bcl`; `python3 scripts/clm_env.py --check` reports none missing. `Box_User_Id__c` (or `Enterprise_Id__c`) and `Allowed_Folder_Ids__c` bring up the preview; `Contracts_Root_Folder_Id__c`, `Clause_Library_Hub_Id__c`, `Counter_Position_Template_ID__c` and `Demo_Signer_Email__c` are what the risk search, clause-library, memo and signature beats each refuse without. See [Deployment](deployment.md) |
