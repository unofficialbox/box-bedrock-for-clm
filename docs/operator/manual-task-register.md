...
| MT-047 | Give the workspace an authenticated Salesforce user before expecting a conversation | Demo owner + Salesforce admin | Required for live Agentforce | The client embeds and Lightning Out reports ready on the site, but its calls return 401 as the guest user. No app deployment is needed: `appId` is an optional Lightning Out 2.0 id from **Setup → Lightning Out 2.0 App Manager**, required only for apps created after Spring '26 |
