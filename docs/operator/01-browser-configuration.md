...
For Governed Workflow, configure only the Agentforce actions used by the Box-led path. For Agentic Orchestration, follow [Agentic Orchestration deployment boundary](03-agentic-deployment.md). The repository provides a local deterministic trace; it does not yet automate managed AgentCore or Databricks provisioning.
