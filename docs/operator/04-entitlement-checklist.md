...
| Box Forms | Governed Workflow | Forms builder | Forms surface opens and allows an unpublished draft |
| Box Apps | Governed Workflow | Apps builder | Apps builder opens and supports metadata views/charts |
| Box Automate | Governed Workflow | Workflow admin | Automate builder exposes Form trigger, Extract Agent, Box Agent, approval, conditional split, and HTTPS Request |
...
| Salesforce API-only integration user | Governed Workflow | Salesforce admin | Minimum-access integration profile/license is available |
| Salesforce External Client App | Governed Workflow | Salesforce admin | External Client App Manager allows client credentials and Run As configuration |
...
| Salesforce UI Bundle | Agentic Orchestration | Salesforce admin | UI Bundle metadata can deploy and be placed on the intended page |
