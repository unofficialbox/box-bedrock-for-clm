...
> **Inbound email is the realistic first hop.** Before a contract reaches the Box intake, it usually arrives by email. The `EmailIntakeHandler` inbound email service (see [Inbound Email Intake Service](../operator/email-intake-service.md)) captures a counterparty's email onto the matching Opportunity's activity timeline and saves the attachment as a Salesforce File; a follow-on step pushes that file into Box `01 - Intake`, where the metadata trigger below takes over. That handler does **not** create `CLM_Contract__c` — record creation stays in the governed path described here.
